/*
 * The purpose of this program is to take the font data stored
 * in flash memory and write it into EEPROM. We need to do this,
 * because the Arduino IDE does not currently write EEPROM data
 * when it uploads a new sketch.
 *
 */
...
}; // 94 characters @ 5 bytes each = 470 bytes
...
void setup()
{
  ...
  for (int i = 0; i < 470; i++)
  {
    ...
    uint8_t value1 = pgm_read_byte(&flash_font[i]);
    EEPROM.update(i, value1);
    digitalWrite(led, HIGH);
  }
...
void loop()
{
  // Iterate through EEPROM and compare to flash memory
  // If they are equal, keep the LED lit
  // If they are not, then turn off the light

  for (uint8_t i = 0; i < 470; i++)
  {
    uint8_t value1 = EEPROM.read(i);
    uint8_t value2 = pgm_read_byte(&flash_font[i]);
